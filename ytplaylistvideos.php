...
$playlist = $youtube->getPlaylistById($thedata['playlistid']);
if (!$autoSave) {
        echo '
...
<div class="cards">
<div class="card youtubepresentation"  style="background-image: url(' . addslashes($playlist->snippet->thumbnails->medium->url) . '); background-size: cover;">';
echo '<h5>Playlist import</h5>
<h1> ' . _html($playlist->snippet->title) . '</h1>';
echo '<h4> by ' . _html($playlist->snippet->channelTitle) . '</h4> ';
echo '</div>';
$videos = array();

    if (!$autoSave) {
        /* List Videos for choosing */
        $videos = $youtube->ListPlaylistItems($thedata['playlistid'], $thedata['pageToken']);
    } else {
        /* List Videos and save them automaticaly to database */
        $videos = $youtube->savePlaylistItems($thedata['playlistid'], $thedata['pageToken']);
    }
